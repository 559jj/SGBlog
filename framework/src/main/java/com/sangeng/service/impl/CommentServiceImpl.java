...
public class CommentServiceImpl extends ServiceImpl<CommentMapper, Comment> implements CommentService {
    ...
    //封装commentVo并将UserName和ToCommentUserName属性赋值
    private List<CommentVo> toCommentVoList(List<Comment> records){
        ...
        for (CommentVo commentVo : commentVos) {
            ...
            String nickName = userService.getById(commentVo.getCreateBy())
                    .getNickName();
            commentVo.setUserName(nickName);
            //通过toCommentUserId查询用户的昵称并赋值
            //如果toCommentUserId不为-1才进行查询
            if (commentVo.getToCommentUserId()!=-1){
                String ToCommentUseName = userService.getById(commentVo.getToCommentUserId()).getNickName();
                commentVo.setToCommentUserName(ToCommentUseName);
            }
        }
